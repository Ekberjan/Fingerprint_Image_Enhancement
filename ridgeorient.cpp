//Main function for estimating orientation
//field of fingerprint ridges
...
cv::Mat RidgeOrient::run(cv::Mat im, double gradientsigma, double blocksigma, double orientsmoothsigma)
{
	...
	cv::filter2D(im, grad_x, -1, fx, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT); //Gradient of the image in x
	cv::filter2D(im, grad_y, -1, fy, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT); //Gradient of the image in y
	...
	for (int i = 0; i < sin2theta.rows; i++)
	{
		...
		{
			float sinPix = sin2theta.at<float>(i, j);
			float cosPix = cos2theta.at<float>(i, j);
			float pix = pi / 2 + std::atan2(sinPix, cosPix) / 2;
			orientim.at<float>(i, j) = pix;
		}
	}
...
void RidgeOrient::gradient(cv::Mat image, cv::Mat xGradient, cv::Mat yGradient)
{
	...
	for (int i = 1; i < image.rows - 1; i++)
	{
		...
		{
			float xPixel1 = image.at<float>(i, j - 1);
			float xPixel2 = image.at<float>(i, j + 1);
			...
			if (j == 0)
			{
				xPixel1 = image.at<float>(i, j);
				xGrad = xPixel2 - xPixel1;
			}
			else if (j == image.cols - 1)
			{
				xPixel2 = image.at<float>(i, j);
				xGrad = xPixel2 - xPixel1;
			}
			else
			{
			...
			if (i == 0)
			{
				yPixel1 = image.at<float>(i, j);
				yGrad = yPixel2 - yPixel1;
			}
			else if (i == image.rows - 1)
			{
				yPixel2 = image.at<float>(i, j);
				yGrad = yPixel2 - yPixel1;
			}
			else
			{
